...
class ModelControler():
    ...
    def test(self):
        ...
        # Initialize a new run depending on the weights used
        if self.opt.best_weights == 'IoU':
            self.initialize_new_run("UNet-test-IoU-run")
        elif self.opt.best_weights == 'Dice':
            self.initialize_new_run("UNet-test-Dice-run")
        elif self.opt.best_weights == 'Pixel_Accuracy':
            self.initialize_new_run("UNet-test-Pixel-Accuracy-run")
        else:
            raise ValueError("Invalid best weights")
